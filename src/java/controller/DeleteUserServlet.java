 ...
 * @author Admin
 */
@WebServlet(name="DeleteUserServlet", urlPatterns={"/management/users/delete"})
public class DeleteUserServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            int id = Integer.parseInt(userIdRaw);
            UserDAO udb = new UserDAO();
            if(udb.deleteUserById(id) > 0) {
                response.sendRedirect("/management");
            } else {
                request.setAttribute("error", "Something wrong from server!");
